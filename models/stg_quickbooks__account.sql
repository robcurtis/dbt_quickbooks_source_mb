        ...
        OR (
            sub_account = true AND (
                ...
                THEN (
                    SELECT MAX(id)  -- Using MAX to ensure single result
                    FROM account a
                    WHERE a.source_relation = account.source_relation  -- Qualify the source_relation
                        AND cast(a.account_number as {{ dbt.type_string() }}) = '1100'  -- Cast for consistency
                        AND a.active = true  -- Ensure account is active
                    )
                ELSE cast(parent_account_id as {{ dbt.type_string() }}) 
                END as parent_account_id
        ) in (
